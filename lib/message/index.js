...
var getOrFetch = function getOrFetch(obj, key, fetch) {
	if (obj && obj[key]) {
		return obj[key];
	}
	var promise = (0, _util.toPromise)(fetch);
	if (promise) {
...
var Message = function Message(props) {
	...
	var fetchUser = data.fetchUser || props.fetchUser;
	fetchUser = (0, _util.isPromise)(fetchUser) || _lodash2.default.isFunction(fetchUser) ? (0, _util.promiseOnce)(fetchUser) : null;

	var avatar = getOrFetch(user, 'avatar', fetchUser);
	var userName = getOrFetch(user, 'name', fetchUser);
